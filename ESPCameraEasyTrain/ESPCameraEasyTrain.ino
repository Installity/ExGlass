...
#include "WiFi.h"
#include "SD_MMC.h"   // For built-in SD card slot on ESP32-CAM (FAT32 formatted)
#include "FS.h"

// Define your camera model; for AI Thinker boards:
#define CAMERA_MODEL_AI_THINKER
#include "camera_pins.h"

// WiFi credentials (CHANGE TO YOUR DESIRED NETWORK)
const char* ssid = "";
const char* password = "";
...
// Flash control
const int flashPin = 4; // Adjust according to your wiring
bool flashEnabled = false;  // Default: flash off
...
void setup() {
  ...
  // Initialize flash pin and force flash off
  ...
  // Initialize SD card and create folder structure
  initSD();
...
void loop() {
  ...
  if (req.startsWith("GET /stream")) {
    ...
    // Ensure flash remains off if flash is disabled.
    if (!flashEnabled) {
      digitalWrite(flashPin, LOW);
      ...
      delay(50);  // Short delay to allow flash to light up
    }
    ...
    // sends a response with meta-refresh to redirect back to main page after 1 second
    ...
    toggleFlash();
    // Respond with new flash state and auto-redirect after 1 second
    ...
    // serves the main homepage with live preview and buttons
...
//
// --- SD Card and Directory Handling ---
//
...
//
// --- Build Main Webpage HTML ---
//

String buildMainPage() {
  String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'>";
  html += "<meta name='viewport' content='width=device-width, initial-scale=1'>";
  html += "<title>Data Training Capture</title>";
  ...
  // Append a random query parameter to force a fresh reload of the MJPEG stream
